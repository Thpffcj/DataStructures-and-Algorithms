 ...
 *
 * 根据每日 气温 列表，请重新生成一个列表，对应位置的输入是你需要再等待多久温度才会升高超过该日的天数。如果之后都不会升高，请在该
 * 位置用 0 来代替。
...
public class DailyTemperatures {
    ...
    /**
     * 我们按逆序遍历列表，对于每个 T[i]，我们要知道 (T[i],100] 温度所出现的位置，为此我们用一个 next 数组记录该数据，若当前
     * 位置出现 T[i]=100，则我们将该索引记录在 next[100]。
